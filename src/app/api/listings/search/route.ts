...
export async function POST(req: Request) {
  try {
    ...
    if (error) {
      ...
      return NextResponse.json({ error: error.message, listings: [] }, { status: 400 });
    }
    ...
    console.error("[search] unhandled", e);
    return NextResponse.json({ error: msg, listings: [] }, { status: 500 });
  }
}
